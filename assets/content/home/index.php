...
$totalReceipts = getReceiptsByDateAll($link, $today);
$creditSales = $totalSales - $totalReceipts;
// Fetch data for cash and credit card payments
$query = "SELECT SUM(`amount`) as sum, type FROM transaction_receipt WHERE `type` IN (0, 1) AND DATE(`current_time`) = '$today' AND `is_active` = 1 GROUP BY type";
$result = mysqli_query($link, $query);
