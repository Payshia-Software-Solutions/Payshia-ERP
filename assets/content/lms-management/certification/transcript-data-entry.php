...
$sql = "SELECT COUNT(id) FROM `assignment_submittion` WHERE created_by LIKE '$indexNo'";
$sql_result = $lms_link->query($sql);
if ($sql_result->num_rows > 0) {
    while ($row = $sql_result->fetch_assoc()) {
        $assignment_count = $row['COUNT(id)'];
    }
}

$sql = "SELECT `id`, `assignment_id`, `file_path`, `created_by`, `created_at`, `status`, `grade` FROM `assignment_submittion` WHERE created_by LIKE '$indexNo'";
