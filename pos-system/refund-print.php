...
$LanguageMode = "EN";
$netTotal = $total = 0;
$refundNumber = $_GET['refundNumber'];
...
$SelectedArray = GetRefundList()[$refundNumber];
$Products = GetProducts($link);
$Units = GetUnit($link);

$barcode = GenerateHighResolutionBarcode($refundNumber);
$textArray = [
    "invoice" => ["EN" => "Refund", "SI" => "ආපසු ගෙවීම"],
...
$inv_time = date("Y-m-d H:i:s", strtotime($SelectedArray['refund_datetime']));


$InvoiceNumber = $SelectedArray['rtn_number'];
$LocationName = GetLocations($link)[$SelectedArray['current_location']]['location_name'];
$selectedLocation =  GetLocations($link)[$SelectedArray['current_location']];

// echo $invAmount;
$customer_code = $SelectedArray['customer_id'];
$CustomerName = GetCustomerName($link, $customer_code);
$created_by =  $SelectedArray['update_by'];
...
$LanguageMode = GetSetting($link, $SelectedArray['current_location'], 'invoiceLang');
...
<body>
    ...
    <div class="inv" id="inv">
        ...
        <div class="InvoiceID" <?= $fontClass ?>><?= $textArray['invoice_number'][$LanguageMode] ?> : <span class="invoice_number"><?= $refundNumber; ?></span></div>
        ...
        <table class="totals">
            ...
            <tr>
                <th colspan="2" <?= $fontClass ?>>
                    Refund Amount</th>
                <th class="text-right" <?= $fontClass ?>><?php echo number_format($SelectedArray['refund_amount'], 2); ?></th>
            </tr>
        </table>
        <hr>
